...
export default function SettingsPage() {
  ...
  const handleRemoveKey = async () => {
    if (!user) return;
    if (!confirm("등록된 Gemini API 키를 삭제할까요?")) return;
    setSavingKey(true);
    try {
      const { getClientDb } = await import("@/lib/firebase/client");
      const db = getClientDb();
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-semibold">Gemini API 키</h3>
            </div>
            {hasKey ? (
          ...
          <p className="text-sm text-muted-foreground mb-3">
            음성 일기·AI 코칭은 <span className="font-medium text-foreground">본인의 Gemini API 키</span>로 동작합니다.
            키는 본인 계정에만 저장되며 다른 사용자는 볼 수 없습니다.
          </p>
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              placeholder={hasKey ? "새 키 입력 (교체 시)" : "AIza... 형식의 키 붙여넣기"}
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-muted/20">
            ...
            <div className="flex items-center gap-3">
              <Badge variant="outline" className="text-xs">
                {profile?.notifyPrefs?.time ?? "07:00"}
              </Badge>
              <Button
                variant="outline"
                size="sm"
